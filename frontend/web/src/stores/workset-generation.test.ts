...
function makeEventStream(terminal: GenerationEvent['type'], extra: Record<string, unknown> = {}): ApiClientContract['streamGenerationEvents'] {
  return vi.fn((_worksetId: string, _operation: string, _generationId: string, _signal: AbortSignal) =>
    events([
      {
...
describe('workset generation store', () => {
  ...
  it('applies snapshot and progress events then confirms the completed terminal', async () => {
    ...
    await store.attach('ws-1', 'conversion', 'gen-1', api)
  ...
  it('tracks failed and canceled terminals distinctly', async () => {
    ...
    await failed.attach('ws-1', 'conversion', 'gen-1', apiStub({ streamGenerationEvents: makeEventStream('failed') }))
    expect(failed.status).toBe('failed')
    expect(failed.errorCode).toBe('GENERATION_FAILED')

    const canceled = useWorksetGenerationStore()
    await canceled.attach('ws-1', 'conversion', 'gen-2', apiStub({ streamGenerationEvents: makeEventStream('canceled') }))
    expect(canceled.status).toBe('canceled')
    expect(canceled.terminal).toBe('event')
  })

  it('refuses a second attach while streaming', async () => {
    const streamGenerationEvents = vi.fn((_w: string, _o: string, _g: string, signal: AbortSignal) =>
    ...
    const first = store.attach('ws-1', 'conversion', 'gen-1', api)
    // Second attach while streaming must be refused (resolve false) and the
    // stream opened only once.
    const second = await store.attach('ws-1', 'conversion', 'gen-2', api)
  ...
  it('marks a transport terminal when the stream ends without a terminal event', async () => {
    ...
    const store = useWorksetGenerationStore()
    await store.attach('ws-1', 'conversion', 'gen-1', apiStub({ streamGenerationEvents }))
