...
// The library's own scan state (L04): one label for the whole library, read
// from the library record rather than derived per folder row. The backend
// writes exactly "", "completed", "failed" and "canceled".
const scanStatusLabel = computed(() => {
  const status = activeLibrary.value?.last_scan_status ?? ''
  if (status === 'completed') return '上次扫描：已完成'
  if (status === 'failed') return '上次扫描：失败'
  if (status === 'canceled') return '上次扫描：已取消'
  return '尚未扫描'
})
...
<template>
  <section class="flex h-full min-w-0 flex-col bg-background">
    <!-- Toolbar (L02): the library selector and scan/stop come first, at every
         width. Management actions stay beside them on the desktop tiers and
         move into the page's 更多 menu on a phone, which is the only way both
         fit at 390px without shrinking a target below 44px. -->
    <header class="flex min-h-14 shrink-0 flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-border px-4 py-1.5 rail:px-5">
      <div class="w-full min-w-0 rail:w-auto">
        ...
        <p class="truncate text-[11px] text-muted-foreground">每行是根目录下的一个音频文件夹</p>
      </div>
      ...
      <select
        ...
        aria-label="切换媒体库"
        class="ml-auto h-11 min-w-0 max-w-52 flex-1 rounded-md border border-input bg-background px-2.5 font-heading text-[16px] font-semibold focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none rail:h-8 rail:flex-none rail:text-xs"
      ...
      <!-- Scanning is only offered once there is a library to scan; the empty
           state has its own 开始扫描 entry (L02: no fake success, no dead
           button). -->
      <Button
        v-if="activeLibrary"
        data-testid="scan-button"
        class="h-11 shrink-0 rail:h-7"
        variant="outline"
        size="sm"
      ...
      <Button class="hidden rail:inline-flex" variant="outline" size="sm" @click="openAdd">
        <Plus class="size-3.5" />
        添加媒体库
      </Button>
      <!-- The phone 更多 menu carries the secondary actions and the shared
           theme entry (N14, L02). -->
      <div class="shrink-0 rail:hidden">
        <HeaderMenu trigger="more">
          <DropdownMenuItem v-if="activeLibrary" data-testid="edit-library" @select="openEdit">
            <Pencil class="size-4" />
            编辑媒体库
          </DropdownMenuItem>
          <DropdownMenuItem data-testid="add-library" @select="openAdd">
            <Plus class="size-4" />
            添加媒体库
          </DropdownMenuItem>
        </HeaderMenu>
    ...
    <template v-if="activeLibrary">
      <!-- Library identity plus the library-level scan state. That state is one
           fact about the library, so it is shown here once instead of being
           repeated on every row as if each folder had been scanned on its own
           (L04); it stays visible at every width. -->
      <div class="flex min-h-14 shrink-0 items-center gap-3 border-b border-border px-4 rail:px-5">
        <div class="min-w-0">
          <h1 class="truncate font-heading text-lg font-semibold tracking-tight">{{ activeLibrary.name }}</h1>
          <p class="truncate font-mono text-[11px] text-muted-foreground" :title="activeLibrary.root_path">
            {{ activeLibrary.root_path }}
          </p>
        </div>
        <span
          class="ml-auto shrink-0 rounded-full border px-2 py-0.5 text-[10px]"
          :class="
            activeLibrary.last_scan_status === 'failed'
              ? 'border-[var(--danger-border)] text-[var(--danger-ink)]'
      ...
      <div class="min-h-0 flex-1">
        ...
        <div v-else-if="foldersSuccess" class="grid h-full min-h-64 place-items-center px-6 text-center">
          <div class="max-w-sm">
            ...
            <Button class="mt-4 h-11 rail:h-7" size="sm" :disabled="scan.status === 'scanning'" @click="runScan">开始扫描</Button>
          </div>
        </div>
      </div>

      <!-- The bar belongs to the selection: with nothing selected there is no
           empty strip above the bottom bar (N15, L07). -->
      <BatchPlanBar
        v-if="ui.selectedFolderIds.length > 0"
    ...
    >
      <div class="max-w-md rounded-lg border border-dashed border-border bg-card/35 px-8 py-10">
        ...
        <Button data-testid="empty-add-library" class="mt-5 h-11 rail:h-7" size="sm" @click="openAdd">添加媒体库</Button>
      </div>
    </div>
