...
<template>
  ...
  >
    <!-- Fits the shell's remaining height on a short phone screen: the panel
         scrolls, the safe area is paid once at the edge (L08). -->
    <div
      class="my-auto max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-lg border border-border bg-card p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-lg rail:p-5 rail:pb-5"
    >
      <div class="flex items-start justify-between gap-3">
        ...
        <Button
          ...
          size="icon"
          aria-label="关闭"
          data-testid="close-workset-dialog"
          @click="emit('close')"
        >
          <X class="size-4" />
        </Button>
      ...
      <label class="mt-4 block">
        ...
        <input
          ...
          type="text"
          maxlength="120"
          class="mt-1 h-11 w-full rounded-md border border-input bg-background px-3 text-[16px] focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none rail:h-9 rail:text-sm"
          placeholder="例如：夏季整理"
          @input="touched = true"
        />
      </label>
      ...
      <div class="mt-5 flex items-center justify-end gap-2">
        <Button class="h-11 rail:h-7" variant="ghost" size="sm" :disabled="saving" @click="emit('close')">取消</Button>
        <Button
