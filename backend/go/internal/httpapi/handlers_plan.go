...
type workflowRequest struct {
	...
	MemberSettings []memberSettingRequest `json:"member_settings,omitempty"`
}

// memberSettingRequest is one member's batch-draft record (schema 2).
type memberSettingRequest struct {
	MemberID      string                `json:"member_id"`
	Excluded      bool                  `json:"excluded"`
	StepOverrides []stepOverrideRequest `json:"step_overrides,omitempty"`
}

// stepOverrideRequest fully replaces one step's default config for a member.
type stepOverrideRequest struct {
	StepID string          `json:"step_id"`
	Config json.RawMessage `json:"config"`
}

type workflowStepRequest struct {
	// StepID is the stable step-instance identity (workset drafts only;
	// omitted for standalone /plans workflow requests).
