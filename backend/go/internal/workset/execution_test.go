...
type execFixture struct {
	...
	libraryID string
	worksetID string
	members   map[string]workset.MemberView // folder name -> member
	draftJSON string
	draftHash string
}
...
func newExecFixtureWithScan(t *testing.T, scan workset.FolderScan) *execFixture {
	t.Helper()
...
func newExecFixtureWithTask(
	...
	f := &execFixture{
		t:         t,
		repo:      repo,
		svc:       workset.NewService(repo, 1, encodeConcurrency, tasks, scan, nil),
		root:      root,
		libraryID: "lib-1",
		members:   map[string]workset.MemberView{},
	}
	...
	res, err := f.svc.CreateCurrentWorkset(context.Background(), workset.CreateCurrentRequest{
...
func (f *execFixture) draft() *workset.Draft {
	f.t.Helper()
	d, err := f.svc.GetDraft(f.t.Context(), f.worksetID, workset.OperationTypeConversion)
	if err != nil {
...
func (f *execFixture) operation() *workset.OperationView {
	f.t.Helper()
	view, err := f.svc.GetOperation(f.t.Context(), f.worksetID, workset.OperationTypeConversion)
	if err != nil {
...
func (f *execFixture) seedRevision(planID string, comps ...seedComponent) {
	...
	roots := make([]workset.PlanRootRecord, 0, len(comps))
	var compsRecords []workset.PlanComponentRecord
	for i, c := range comps {
		...
		if !seen {
			...
			roots = append(roots, workset.PlanRootRecord{
				RootIndex:            idx,
		...
		compsRecords = append(compsRecords, workset.PlanComponentRecord{
			StepIndex:      0,
			ComponentIndex: i,
...
func (f *execFixture) startExecution(
	planID, key string,
	folders ...string,
) (*workset.StartExecutionResult, error) {
	f.t.Helper()
	return f.svc.StartExecution(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion, planID,
...
func (f *execFixture) mustStart(planID, key string) *workset.ExecutionView {
	f.t.Helper()
...
func (f *execFixture) waitTerminal(executionID string) *workset.ExecutionView {
	f.t.Helper()
	...
	for time.Now().Before(deadline) {
		...
		switch view.Status {
		case workset.ExecStatusSucceeded, workset.ExecStatusFailed,
			workset.ExecStatusCanceled, workset.ExecStatusInterrupted:
			return view
		}
...
func TestStartExecutionGates(t *testing.T) {
	...
	t.Run("version_conflict", func(t *testing.T) {
		...
		_, err := f.svc.StartExecution(
			f.t.Context(), f.worksetID, workset.OperationTypeConversion, "plan-v",
			workset.StartExecutionRequest{IfMatchVersion: 1, IdempotencyKey: "k-v"},
		)
	...
	t.Run("draft_changed", func(t *testing.T) {
		...
		if _, err := f.svc.SaveDraft(
			f.t.Context(), f.worksetID, workset.OperationTypeConversion,
			workset.SaveDraftRequest{Document: draftJSON(t, doc), IfMatchVersion: f.operation().Version},
		); err != nil {
	...
	t.Run("gone_library", func(t *testing.T) {
		...
		_, err := f.svc.StartExecution(
			f.t.Context(), f.worksetID, workset.OperationTypeConversion, "plan-gone",
			workset.StartExecutionRequest{IfMatchVersion: 1, IdempotencyKey: "k-gone"},
		)
		code, _ := errorDetail(t, err)
...
func TestStartExecutionSnapshotGates(t *testing.T) {
	...
	t.Run("zero_operation_revision_succeeds", func(t *testing.T) {
		...
		if done.Status != workset.ExecStatusSucceeded {
...
func TestExecutionComponentFailureStopsAdmission(t *testing.T) {
	...
	if done.Status != workset.ExecStatusFailed {
...
func TestExecutionScopeRefusesAFolderTheRecordDoesNotHold(t *testing.T) {
	...
	if done := f.waitTerminal(started.ExecutionID); done.Status != workset.ExecStatusSucceeded {
		t.Fatalf("status = %s (%s)", done.Status, done.ErrorMessage)
	}
}
...
func TestExecutionConcurrentStartsHaveOneWriter(t *testing.T) {
	...
	type outcome struct {
		res *workset.StartExecutionResult
		err error
	}
...
func TestExecutionBlocksGenerationAndDraftEdits(t *testing.T) {
	...
	_, err := f.svc.StartGeneration(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion,
		workset.StartGenerationRequest{IfMatchVersion: f.operation().Version, IdempotencyKey: "gen-busy"},
	)
	code, _ := errorDetail(t, err)
	...
	_, err = f.svc.SaveDraft(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion,
		workset.SaveDraftRequest{Document: f.draft().Document, IfMatchVersion: f.operation().Version},
	)
	code, _ = errorDetail(t, err)
	if code != "EXECUTION_IN_PROGRESS" {
	...
	canceled, err := f.svc.CancelExecution(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion, queued.ExecutionID,
	)
	if err != nil {
		t.Fatalf("CancelExecution: %v", err)
	}
	if canceled.Status != workset.ExecStatusCanceled {
...
func TestExecutionSubscriptionSnapshotAndTerminalEvent(t *testing.T) {
	...
	go func() {
		...
		_ = f.svc.SubscribeExecution(
			ctx, f.worksetID, workset.OperationTypeConversion, started.ExecutionID,
	...
	for {
		...
		case ev := <-events:
			...
			if ev.name == "succeeded" {
				...
				if len(names) == 0 || names[0] != workset.ExecutionEventSnapshot {
...
func TestExecutionDetailPagesTheComponentList(t *testing.T) {
	...
	firstPage, err := f.svc.GetExecution(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion, started.ExecutionID,
		workset.ExecutionPage{Limit: 2},
	)
	if err != nil {
	...
	secondPage, err := f.svc.GetExecution(
		f.t.Context(), f.worksetID, workset.OperationTypeConversion, started.ExecutionID,
		workset.ExecutionPage{FromIndex: 2},
	)
	if err != nil {
	...
	if secondPage.Components[0].Status != workset.ExecComponentSucceeded {
...
func TestExecutionSubscriptionPushesEachComponentResult(t *testing.T) {
	...
	first := <-events
	if first.name != workset.ExecutionEventSnapshot {
		t.Fatalf("first event = %s, want %s", first.name, workset.ExecutionEventSnapshot)
	}
	components, _ := first.data["components"].([]any)
	if len(components) != 1 {
		t.Fatalf("snapshot components = %+v", first.data["components"])
	}
	if entry, _ := components[0].(map[string]any); entry["status"] != workset.ExecComponentPending {
	...
	for {
		...
		case ev := <-events:
			switch ev.name {
			case workset.ExecutionEventComponent:
				if ev.data["component_index"] != float64(0) ||
					ev.data["status"] != workset.ExecComponentSucceeded {
					t.Fatalf("component event = %+v", ev.data)
				}
				pushed = true
			case workset.ExecutionEventSucceeded:
				if !pushed {
					t.Fatal("the session finished without ever pushing its component result")
				}
				return
			case workset.ExecutionEventFailed, workset.ExecutionEventCanceled:
				t.Fatalf("unexpected terminal event %s: %+v", ev.name, ev.data)
			}
		case <-ctx.Done():
