...
func (s *serviceImpl) rejectActiveSession(op *Operation) (*StartGenerationResult, error) {
...
func (s *serviceImpl) revisionHealth(
	...
	return task.EvaluateRevision(RevisionFacts{Detail: detail})
}
...
func (s *serviceImpl) participatingMembers(input *generationInput) []*WorksetMember {
	...
	out := make([]*WorksetMember, 0, len(participating))
...
func (s *serviceImpl) replayGeneration(
	...
	if existing == nil ||
		(existing.Status != GenStatusCompleted && existing.Status != GenStatusQueued && existing.Status != GenStatusRunning) {
		return nil, false, nil
	}
...
func (s *serviceImpl) persistGeneration(
	...
	if err := s.enqueue(func() error { return s.repo.CreateGeneration(gen) }); err != nil {
		if errors.Is(err, ErrGenerationIdemConflict) {
...
func (s *serviceImpl) CancelGeneration(
	...
	if g.Status == GenStatusRunning || g.Status == GenStatusQueued {
...
func (s *serviceImpl) runGeneration(
	...
	return task.PlanSession(ctx, PlanSessionInput{
		RawDraft: req.Draft,
		Members:  members,
		Progress: progress,
	})
...
func hashMembers(members []*WorksetMember) string {
	h := sha256.New()
