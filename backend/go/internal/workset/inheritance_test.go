...
func (f *fixture) resolve(ws *workset.WorksetView) map[string]resolvedMember {
	f.t.Helper()
...
func (f *fixture) resolvedFrom(ws *workset.WorksetView, planID string) map[string]resolvedMember {
	f.t.Helper()
	rev, err := f.svc.GetRevision(f.ctx, ws.WorksetID, workset.OperationTypeConversion, planID)
	if err != nil {
...
func TestExcludingEveryMemberBlocksGeneration(t *testing.T) {
	...
	if werr, ok := workset.AsError(err); !ok || werr.Code != "NO_ACTIVE_MEMBERS" {
