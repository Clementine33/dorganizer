...
func (s *serviceImpl) replayExecution(
	op *Operation, key, requestHash string,
...
func (s *serviceImpl) executionEligibility(
	op *Operation, planID string, ifMatchVersion int,
...
func (s *serviceImpl) executionBlockReasons(
	op *Operation, planID string, rev *OperationRevision,
) (*PlanDetail, []string, error) {
...
func (s *serviceImpl) freezeExecution(
	...
	frozen, reasons, err := task.FreezeExecution(RevisionFacts{
...
// and opaque here.
func executionOptionsOf(e *PlanExecution) json.RawMessage {
...
func (s *serviceImpl) persistExecution(
	...
	createErr := s.enqueue(func() error {
		return s.repo.CreateExecutionGuarded(exec, ExecutionGuards{
			ExpectedOperationVersion: op.Version,
	...
	if err := createErr; err != nil {
		if errors.Is(err, ErrExecutionIdemConflict) {
		...
		case errors.Is(err, ErrDraftChanged):
...
func (s *serviceImpl) CancelExecution(
	...
	if e.Status == ExecStatusQueued || e.Status == ExecStatusRunning {
...
func (s *serviceImpl) executionViewOf(e *PlanExecution, page ExecutionPage) (*ExecutionView, error) {
	req, err := parseExecutionRequest(e.RequestJSON)
	if err != nil {
...
func applyComponentResult(entry *ExecutionComponentView, res ExecutionComponentResult) {
...
func componentResultRow(entry *ExecutionComponentView) ExecutionComponentResult {
	return ExecutionComponentResult{
...
func executionRefOf(e *PlanExecution) *ExecutionRef {
	return &ExecutionRef{
		ExecutionID:  e.ExecutionID,
		PlanID:       e.PlanID,
		Status:       e.Status,
...
func executionProgressOf(e *PlanExecution) *ExecutionProgress {
