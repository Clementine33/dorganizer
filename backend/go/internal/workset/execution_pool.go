...
func (s *sessionRun) settleHead(
	...
	if entry.Status == ExecComponentCanceled {
		s.stopAfterHead(ExecStatusCanceled, "CANCELED", "execution canceled")
		return true
	}
	return false
}
...
func (s *sessionRun) prepare(u ExecutionUnit, reportIdx int) *sessionStop {
	prepared, err := s.task.PrepareUnit(s.ctx, UnitRunInput{
...
func (s *sessionRun) stopSession(observed sessionStop) {
	...
	if stop.err == nil {
		s.discardOpen(nil)
		s.d.finishExecution(s.ex, ExecStatusCanceled, "CANCELED", "execution canceled")
		return
	}
