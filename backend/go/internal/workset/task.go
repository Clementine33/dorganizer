...
type Task interface {
	...
	ValidateDraft(raw []byte, members []*WorksetMember) error
	...
	ValidateSessionInput(rawDraft []byte, members []*WorksetMember) error
	...
	PlanSession(ctx context.Context, in PlanSessionInput) (*PlanSnapshot, error)
	...
	EvaluateRevision(in RevisionFacts) (RevisionHealth, error)
	...
	// nil error.
	FreezeExecution(in RevisionFacts) (FrozenExecution, []string, error)
	...
	PrepareUnit(ctx context.Context, in UnitRunInput) (PreparedUnit, error)
