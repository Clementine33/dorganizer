...
func (s *serviceImpl) loadWorkset(id string) (*Workset, error) {
	w, err := s.repo.GetWorkset(id)
	if err != nil {
		if errors.Is(err, ErrWorksetNotFound) {
...
func (s *serviceImpl) loadOperation(worksetID, operationType string) (*Operation, error) {
...
func (s *serviceImpl) operationView(op *Operation) (*OperationView, error) {
	out := &OperationView{
	...
	if latest != nil && latest.Status != GenStatusRunning && latest.Status != GenStatusQueued {
