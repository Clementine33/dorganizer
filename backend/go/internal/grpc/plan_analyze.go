...
func (s *OnseiServer) analyzeFolders(folderPaths []string, analyzer *analyze.Analyzer, purePattern *regexp.Regexp, bitrateBatchUpdate bool) (*analyze.Plan, error) {
	if len(folderPaths) == 0 {
		return &analyze.Plan{PlanID: "", Operations: []analyze.Operation{}}, nil
	}
	...
	for _, fp := range folderPaths {
		folderPathPosix := filepath.ToSlash(fp)
		if !strings.HasSuffix(folderPathPosix, "/") {
			folderPathPosix += "/"
		}

		rows, err := s.repo.DB().Query("SELECT path, COALESCE(file_size, size, 0), COALESCE(bitrate, 0), COALESCE(format, '') FROM entries WHERE is_dir = 0 AND path LIKE ?", folderPathPosix+"%")
		if err != nil {
			return nil, err
		}
	...
	if err := analyzer.EnrichScopedEntriesBitrateWithBatchOption(entries, bitrateBatchUpdate); err != nil {
		return nil, fmt.Errorf("enrich scoped entries bitrate: %w", err)
	}
...
func (s *OnseiServer) analyzeFolder(folderPath string, analyzer *analyze.Analyzer, purePattern *regexp.Regexp, bitrateBatchUpdate bool) (*analyze.Plan, error) {
	folderPathPosix := filepath.ToSlash(folderPath)
	if !strings.HasSuffix(folderPathPosix, "/") {
		folderPathPosix += "/"
	}
...
func (s *OnseiServer) analyzeFoldersWithErrors(folderPaths []string, analyzer *analyze.Analyzer, targetFormat string, purePattern *regexp.Regexp, bitrateBatchUpdate bool) (string, []analyze.Operation, []*pb.FolderError, []string, error) {
	if len(folderPaths) == 0 {
	...
	for _, fp := range folderPaths {
		go func(folderPath string) {
			...
			rootPath := ""
			var resolved string
			err := s.repo.DB().QueryRow("SELECT root_path FROM entries WHERE path LIKE ? LIMIT 1", folderPathCanon+"/%").Scan(&resolved)
			if err == nil && resolved != "" {
				rootPath = resolved
			}
			if rootPath == "" {
			...
			rows, err := s.repo.DB().Query("SELECT path, COALESCE(file_size, size, 0), COALESCE(bitrate, 0), COALESCE(format, '') FROM entries WHERE is_dir = 0 AND path LIKE ?", folderPathPosix+"%")
			if err != nil {
				result.err = &pb.FolderError{Stage: "plan", Code: "DB_QUERY_FAILED", Message: fmt.Sprintf("failed to query entries: %v", err), FolderPath: folderPathCanon, RootPath: rootPath}
				results <- result
				return
			}
			...
			for rows.Next() {
				...
				if err := rows.Scan(&e.PathPosix, &e.FileSize, &e.Bitrate, &e.Format); err != nil {
					rows.Close()
					result.err = &pb.FolderError{Stage: "plan", Code: "DB_SCAN_FAILED", Message: fmt.Sprintf("failed to scan entry: %v", err), FolderPath: folderPathCanon, RootPath: rootPath}
					results <- result
					return
				}
				if _, entryErr := validateAndNormalizePath(e.PathPosix); entryErr != nil {
					entryErr.FolderPath = folderPathCanon
					entryErr.RootPath = rootPath
					result.err = entryErr
					rows.Close()
					results <- result
					return
			...
			if len(entries) > 0 {
				...
				if mode == 2 {
					if err := analyzer.EnrichScopedEntriesBitrateWithBatchOption(entries, bitrateBatchUpdate); err != nil {
						result.err = &pb.FolderError{Stage: "plan", Code: "SLIM_ANALYZE_FAILED", Message: fmt.Sprintf("failed to enrich scoped bitrate: %v", err), FolderPath: folderPathCanon, RootPath: rootPath}
						results <- result
						return
					}
					plan = analyze.AnalyzeSlimMode2(entries, purePattern)
				...
				if len(plan.Errors) > 0 {
					...
					result.err = &pb.FolderError{Stage: "plan", Code: firstErr.Code, Message: firstErr.Message, FolderPath: folderPathCanon, RootPath: rootPath}
					results <- result
					return
				}
	...
	for _, item := range ordered {
		...
		if result.err != nil {
			...
			if result.err.RootPath == "" {
				result.err.RootPath = result.rootPath
			}
			result.err.Timestamp = timestamp
			result.err.EventId = generateEventID()
...
func (s *OnseiServer) analyzeFolderWithErrors(folderPath string, analyzer *analyze.Analyzer, targetFormat string, purePattern *regexp.Regexp, bitrateBatchUpdate bool) (string, []analyze.Operation, []*pb.FolderError, []string, error) {
	rootPath, ops, errors, successes, err := s.analyzeFoldersWithErrors([]string{folderPath}, analyzer, targetFormat, purePattern, bitrateBatchUpdate)
	return rootPath, ops, errors, successes, err
}

func validateAndNormalizePath(path string) (string, *pb.FolderError) {
	if strings.ContainsRune(path, '\x00') {
		return "", &pb.FolderError{Stage: "plan", Code: "PATH_NULL_BYTE", Message: "path contains null bytes which are invalid"}
	}
	...
	for _, part := range parts {
		if len(part) > 255 {
			if utf8.RuneCountInString(part) > 255 {
				return "", &pb.FolderError{Stage: "plan", Code: "PATH_NODE_RUNE_TOO_LONG", Message: fmt.Sprintf("path component exceeds 255 runes: %s...", part[:50])}
			}
			return "", &pb.FolderError{Stage: "plan", Code: "PATH_NODE_BYTE_TOO_LONG", Message: fmt.Sprintf("path component exceeds 255 bytes: %s...", part[:50])}
		}
	}
	...
	if err != nil {
		return "", &pb.FolderError{Stage: "plan", Code: "PATH_ABS_FAILED", Message: fmt.Sprintf("failed to get absolute path: %v", err)}
	}

	return filepath.ToSlash(filepath.Clean(absPath)), nil
}

func (s *OnseiServer) collectPruneEntriesByScopes(folderPaths []string, folderPath string, sourceFiles []string) ([]analyze.Entry, error) {
	seen := make(map[string]struct{})
	entries := make([]analyze.Entry, 0)
...
func (s *OnseiServer) collectSlimEntriesByScopes(folderPaths []string, folderPath string, sourceFiles []string) ([]analyze.Entry, error) {
	return s.collectPruneEntriesByScopes(folderPaths, folderPath, sourceFiles)
}
