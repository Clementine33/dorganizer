...
// Gate serializes the two kinds of disk access that must not overlap: the
// managed task paths (scan, planning, execution) and direct file management.
//
...
// afterwards sees the session instead of racing it, and vice versa.
//
...
// file management holds the slot. The write itself happens inside the critical
// section: a file management request that arrives after it sees the queued
// session in the database, never a half-registered one.
