...
func seedFeedFixtures(t *testing.T, svc *serviceImpl) {
	...
	// pending: unplanned
	if _, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: "待规划", FolderIDs: []string{"f-a"}, IdempotencyKey: "feed-1"}); err != nil {
	...
	if _, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: "正常", FolderIDs: []string{"f-a"}, IdempotencyKey: "feed-2"}); err != nil {
		t.Fatalf("create normal: %v", err)
	}
	// error: orphaned
	res, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: "孤立", FolderIDs: []string{"f-a"}, IdempotencyKey: "feed-3"})
	if err != nil {
		t.Fatalf("create orphan: %v", err)
	}
	if _, err := repo.DB().Exec("UPDATE worksets SET library_id = NULL WHERE id = ?", res.Workset.WorksetID); err != nil {
	...
	res4, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: "阻塞", FolderIDs: []string{"f-a"}, IdempotencyKey: "feed-4"})
	if err != nil {
...
func TestFeedFilterPaginationAcrossPages(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		if _, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: fmt.Sprintf("p%d", i), FolderIDs: []string{"f-a"}, IdempotencyKey: fmt.Sprintf("pg-%d", i)}); err != nil {
			t.Fatalf("create %d: %v", i, err)
		}
	}
	for i := 0; i < 2; i++ {
		res, err := svc.CreateWorkset(ctx, CreateRequest{LibraryID: "lib-1", Title: fmt.Sprintf("o%d", i), FolderIDs: []string{"f-a"}, IdempotencyKey: fmt.Sprintf("og-%d", i)})
		if err != nil {
