...
func TestExecutionRevalidatesInputsAfterRefresh(t *testing.T) {
	...
	if done.Status != sqlite.ExecStatusFailed || done.ErrorCode != worksetusecase.ExecBlockedInput {
		t.Fatalf(
...
func TestExecutionFailsWhenTheRefreshFails(t *testing.T) {
	...
	if done.Status != sqlite.ExecStatusFailed || done.ErrorCode != "SCAN_FAILED" {
